...
from app.models.schema import VideoParams, VideoAspect, VoiceNames, VideoSource
from app.services import task as tm

st.set_page_config(page_title="AutoVideoToolsByMoneyPrinter", page_icon="🤖", layout="wide",
...
with left_panel:
    with st.container(border=True):
        ...
        video_sources = [
            ("抖音", VideoSource.douyin.value),
            ("Pexels", VideoSource.pexels.value),
            ("Youtube", VideoSource.youtube.value),
            ("Tiktok", VideoSource.tiktok.value),
        ]
        selected_index = st.selectbox("视频素材来源",
                                      options=range(len(video_sources)),  # 使用索引作为内部选项值
                                      format_func=lambda x: video_sources[x][0]  # 显示给用户的是标签
                                      )
        cfg.video_sources = VideoSource(video_sources[selected_index][1])
        # 视频获取条数
        video_amounts = range(2, 10)  # 允许用户选择的视频条数范围，从1到10
        selected_amount = st.selectbox("选择下载视频的条数",
                                       options=video_amounts  # 直接使用video_amounts作为选项
                                       )
        cfg.video_amounts = selected_amount  # 假设你有一个配置对象cfg用于存储这些配置信息
