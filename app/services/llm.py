...
from app.models.schema import VideoSource
...
## Goals:
Generate a script for a video Summarize and summarize into short video copy, depending on the subject of the video.
...
def generate_terms(video_subject: str, video_script: str, video_sources: str, amount: int = 5) -> List[str]:
    if video_sources == VideoSource.douyin.value:
        lanuage = 'Chinese'
    else:
        lanuage = 'English'
    prompt = f"""
...
5. reply with {lanuage} search terms only.
