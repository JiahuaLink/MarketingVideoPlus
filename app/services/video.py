...
def combine_videos(combined_video_path: str,
    ...
    logger.info(f"正在组合视频片段{len(video_paths)} 至{combined_video_path}")
    audio_clip = AudioFileClip(audio_file)
    max_duration = audio_clip.duration
    logger.info(f"音频的最大持续时间：{max_duration}秒")
    # Required duration of each clip
    req_dur = max_duration / len(video_paths)
    logger.info(f"每个音频的最大长度为{req_dur}秒")
    ...
    while tot_dur < max_duration:
        for video_path in video_paths:
            ...
            if clip.w != video_width or clip.h != video_height:
                ...
                logger.info(f"将视频大小调整为 {video_width} x {video_height}")
    ...
    logger.info(f"开始写入视频")
    final_clip.write_videofile(combined_video_path, threads=threads)
    logger.success(f"视频片段合成完成")
...
def wrap_text(text, max_width, font='Arial', fontsize=60):
    ...
    logger.warning(f"使用textwrap尝试分行，检查每行限制最大宽度: {max_width}, 字体宽度: {width}, 字体: {text}")
    _wrapped_lines_ = []
...
def generate_video(video_path: str,
    ...
    logger.info(f"  ① 视频素材: {video_path}")
    logger.info(f"  ② 文案音频: {audio_path}")
    logger.info(f"  ③ 文案字母: {subtitle_path}")
    ...
    if bgm_file:
        logger.info(f"添加背景音乐ing: {bgm_file}")
